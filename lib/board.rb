class Board
  ...
  def render(display = false)
    ...
    alphabetical_length  = calculate_row_letter(@rows).length
  ...
  def create_board(columns, rows)
    ...
    character_range.each do |w|
      character_value = calculate_row_letter(w)
  ...
  def calculate_row_letter(num)
    if num == 0
      ...
      calculate_row_letter(num / 26 - divisible) + calculate_row_letter(num % 26)
    end
  end
  ...
  def construct_row_render(row_number, columns, padding, display)
    row = ""
    number_string_length = @columns.to_s.length
    letter = calculate_row_letter(row_number)
